# -*- coding: utf-8 -*-
"""批量快照更新到 2026-07-17 + 周线聚合"""
import json, re, urllib.request, time, subprocess
...
for fc in ["512890","515910","512750","159399"]:
    if fc not in codes: codes.append(fc)
...
for m in re.finditer(r'hq_str_(?:sh|sz)(\w+)="([^"]+)"', raw_text):
    ...
    date_str = next((fields[fi] for fi in range(-6,-1)
                     if re.match(r"\d{4}-\d{2}-\d{2}", fields[fi])), None)
    if not date_str: continue
    try:
        spot_data[code] = {"date":date_str,"open":float(fields[1]),"prev":float(fields[2]),
            "low":float(fields[3]),"high":float(fields[4]),"close":float(fields[5]),
            "vol":int(float(fields[12] if len(fields)>12 else fields[7]))}
    except: continue

print(f"[Phase1] 快照: {len(spot_data)}/{len(codes)}", flush=True)
...
for code in codes:
    ...
    try:
        ...
        print(f"  + {code} {s['date']} C={s['close']:.4f} ({pct:+.2f}%)", flush=True)
        updated1 += 1
    except Exception as e:
        print(f"  ERR {code}: {e}", flush=True)

print(f"[Phase1] updated={updated1}", flush=True)

# Phase2: 159xxx
missing159 = [c for c in codes if c.startswith("159") and c not in spot_data]
if missing159:
    print(f"[Phase2] 补159xxx ({len(missing159)}只)...", flush=True)
    ...
    today = json.loads(urllib.request.urlopen(
        urllib.request.Request(pu, headers=UA), timeout=15
    ).read().decode("utf-8"))[-1]["day"].split()[0]
    updated2 = 0
    for code in missing159:
        try:
            ...
            klines = json.loads(urllib.request.urlopen(
                urllib.request.Request(ku, headers=UA), timeout=20
            ).read().decode("utf-8"))
            ...
            nr = {"date":today,"open":round(float(daily["open"]),4),
                ...
                "amount":0,"chg":0.0}
            recs.append(nr); obj["records"] = recs; obj["update"] = today
            with open(hf,"w",encoding="utf-8") as f:
                json.dump(obj,f,ensure_ascii=False,indent=None,separators=(",",":"))
            pct = (nr["close"]/nr["open"]-1)*100 if nr["open"] else 0
            print(f"  + {code} {today} C={nr['close']:.4f} ({pct:+.2f}%)", flush=True)
            updated2 += 1
        except Exception as e:
            print(f"  ERR {code}: {e}", flush=True)
        time.sleep(0.12)
    print(f"[Phase2] updated={updated2}", flush=True)

print("=== 日线更新完成 ===", flush=True)
