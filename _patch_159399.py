# -*- coding: utf-8 -*-
"""单独补159399今日数据 (Sina kline接口) + 补齐其余59只快照未覆盖的ETF"""
...
def sina_kline_last(code, n=5):
    """用Sina kline接口取最近n条，返回[(date,open,high,low,close,vol)]"""
...
def append_one(code, kline_data, today):
    """将kline_data中今日那条追加到历史文件"""
    hf = HISTORY / f"{code}.json"
    if not hf.exists():
        print(f"  NOFILE {code}"); return False
    ...
    if not today_rec:
        print(f"  NOTODAY {code}: no {today} in kline"); return False
    if last_date == today:
        print(f"  SKIP  {code}: already {today}"); return False
    ...
    pct = (today_rec[4] / today_rec[1] - 1) * 100 if today_rec[1] else 0
    print(f"  + {code} {today} C={today_rec[4]:.4f} V={today_rec[5]:,}")
    return True

def main():
    print(f"Codes to patch: {len(MISSING_CODES)}")
    # 先确认今日日期
    today = "2026-07-13"
    print(f"Target date: {today}")

    # Probe: get today's date from Sina
    probe = sina_kline_last("510500", n=3)
    if probe:
        today = probe[-1][0]
        print(f"Sina confirms today = {today}")
    ...
    for code in MISSING_CODES:
        try:
            ...
            if not klines:
                print(f"  EMPTY {code}: no kline data")
                continue
