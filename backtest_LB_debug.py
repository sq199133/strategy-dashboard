...
def backtest(lb, label, debug=False):
    ...
    for idx, w in enumerate(sorted_weeks):
        ...
        if debug and idx < 5:
            print(f'  Week {w}: val={val:.0f}, cash={cash:.0f}, pos={list(shares.keys())}')
        ...
        if cash > 0 and top3:
            ...
            for c, mom, price in top3:
                ...
                if n > 0:
                    ...
                    cash -= n * price
            if debug and idx < 5:
                print(f'    buy target={[x[0] for x in top3]}, alloc={alloc:.0f}, cash_after={cash:.0f}')
