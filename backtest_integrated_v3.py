...
def calc_factors(code, wk_list, idx, series, ohlc, atr, vol_div_n=None):
    ...
    # v4.7: MA21硬过滤 — 价格必须在MA21上方（不要求ma5>ma21）
    if price <= ma21: return res
    sig_week = srt[idx][0]
...
def run_oos(etfs, series, ohlc, code_cat, all_weeks, atr, is_end_idx, oos_end_idx,
             ...
             no_dedup=False, skip_weeks=None):
    ...
    skip_set = set(skip_weeks) if skip_weeks else set()
    prev_equity_for_skip = None  # carry real equity through skip weeks
    for si in range(is_end_idx, max(oos_end_idx - 1, is_end_idx)):
        if si in skip_set:
            exec_week = all_weeks[si + 1]
            real_eq = cash
            for code, pos in list(portfolio.items()):
                p = next((cl for wk, cl in series[code] if wk == exec_week), None)
                if p: real_eq += pos["weight"] * p
            prev_equity_for_skip = real_eq
            continue
        ...
        equity = cash + sum(
            pos["weight"] * next((cl for wk, cl in series[c] if wk == exec_week),
                                   pos.get("_prev_exec_price", pos["buy_price"]))
            for c, pos in portfolio.items())
        # Update prev exec price for next potential skip
        if prev_equity_for_skip is not None:
            for code, pos in portfolio.items():
                p = next((cl for wk, cl in series[code] if wk == exec_week), None)
                if p: pos["_prev_exec_price"] = p
            prev_equity_for_skip = None
...
def main():
    ...
    # Configs: (label, enable_vol_div, atr_w, vol_div_n, use_ksft, ksft_bonus, no_dedup)
    configs = [
        ("Baseline(C+0.02)",            False, "equal",    0, False, 0.0, False),
        ("Baseline(C+0.02) NoDedup",     False, "equal",    0, False, 0.0, True),
        ("+VD=4(VolDiv)",               True,  "equal",    4, False, 0.0, False),
        ("NoDedup+VD=4",                 True,  "equal",    4, False, 0.0, True),
    ]

    sep70 = "#" * 70
    print("\n" + sep70 + "\n  # IS (2017 - 2022)\n" + sep70)
    is_results = []
    for label, evd, eaw, vdn, uks, kbf, nd in configs:
        print("  {:30s}".format(label), end="", flush=True)
        r = run_oos(etfs, series, ohlc, code_cat, all_weeks, atr, is_start, is_end,
                    evd, vdn, eaw, uks, kbf, nd)
        if r:
            is_results.append((label, evd, eaw, vdn, uks, kbf, nd, r))
    ...
    print("\n" + sep70 + "\n  # OOS (2023 - 2026)\n" + sep70)
    oos_results = []
    for label, evd, eaw, vdn, uks, kbf, nd, r in is_results:
        print("  {:30s}".format(label), end="", flush=True)
