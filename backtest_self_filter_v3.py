...
if __name__ == '__main__':
    ...
    # 新增测试：更严格的三周阈值
    new_cases = [
        ('G8:  三周≥5%  + 本周≥-1%',           0.05, -0.01),
        ('G9:  三周≥5%  + 本周≥0%',            0.05,  0.00),
        ('G10: 三周≥10% + 本周≥-1%',           0.10, -0.01),
        ('G11: 三周≥5%  + 本周≥-2%',           0.05, -0.02),
        ('G12: 三周≥7%  + 本周≥0%',            0.07,  0.00),
    ]

    results = []
    for label, m3, m1 in new_cases:
        tag3 = f'{m3*100:+.0f}%'
    ...
    # 和之前的最优结果合并，一起排序
    prev = json.load(open(os.path.join(OUTPUT_DIR, 'bt_self_filter_v2_20260605_233414.json'), encoding='utf-8'))
    for item in prev:
        results.append((item['label'], {k:v for k,v in item.items() if k != 'label'}))

    results.sort(key=lambda x: x[1]['sharpe'], reverse=True)
    print(f'\n{"="*70}')
    print(f'  全部条件对比（按夏普排序）')
    print(f'{"="*70}')
    ...
    out = os.path.join(OUTPUT_DIR, f'bt_self_filter_v3_{ts}.json')
