...
# -*- coding: utf-8 -*-
"""
v4.0 Weekly Momentum Backtest (v4 - no look-ahead)
- Signal on week T (Friday close), execute on week T+1 (Monday open ≈ T+1 close)
- Uses fractional shares (net value method)
- Equal weight, top-N, category dedup
- Stop loss: -8% cost, -10% HWM
- Word export: backtest results saved as .docx
"""
...
def export_to_word(args, label, total_ret, ann_ret, max_dd, sharpe, calmar, win_rate,
    ...
    # Title
    t = doc.add_heading('周线动量策略 回测报告', 0)
    ...
    # Save
    fname = f'bt_v4_{args.ma_s}_{args.ma_l}_{args.lb}_{int(args.max_dev)}_{args.top_n}_{ts}.docx'
...
def main():
    ...
    fname = f'bt_v4_{args.ma_s}_{args.ma_l}_{args.lb}_{int(args.max_dev)}_{args.top_n}_{ts}.json'
    fp = os.path.join(OUTPUT_DIR, fname)
