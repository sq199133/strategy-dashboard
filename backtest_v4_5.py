...
def main():
    ...
    ap.add_argument('--volume-rank', type=float, default=0.5, help='成交量排名阈值（0.5=Top 50%%）')
    ap.add_argument('--output', type=str, default=None)
    args = ap.parse_args()

    label = f"MA{args.ma_s}/{args.ma_l} LB{args.lb} D{args.max_dev} H{args.top_n} VolRank{args.volume_rank}"
    print(f"{'='*60}")
    print(f"  v4.5 Backtest (Volume Rank Filter): {label}")
    print(f"  Range: {args.start} ~ {args.end}")
    print(f"  Volume Rank: Top {args.volume_rank*100:.0f}%")
    print(f"{'='*60}\n")

    etfs = load_pool()
    print("Loading ETF data with volume...")
    ...
    for etf in etfs:
        ...
        if data and len(data) >= 30:
            ...
            all_series[code] = list(zip(weeks, closes))
            all_volumes[code] = dict(zip(weeks, volumes))
    ...
    def volume_at(code, week):
        return all_volumes.get(code, {}).get(week, 0)

    def get_volume_rank(code, week):
        """Get volume rank of this ETF among all ETFs at this week."""
        if code not in all_volumes or week not in all_volumes[code]:
            return 0
        my_vol = all_volumes[code][week]
        if my_vol <= 0:
            return 0

        # Get all volumes at this week
        all_vols = []
        for c in all_series:
            if week in all_volumes.get(c, {}):
                v = all_volumes[c][week]
                if v > 0:
                    all_vols.append(v)
        ...
        # Rank: what percentage of ETFs have lower volume than this one
        rank = sum(1 for v in all_vols if v < my_vol) / len(all_vols)
        return rank

    # Signal function with volume rank filter
    def get_signal(code, week):
        """Return signal if passes all filters including volume rank."""
        ...
        # Volume rank filter
        vol_rank = get_volume_rank(code, week)
        if vol_rank < args.volume_rank:
            return None

        return {'code': code, 'close': price, 'mom': mom, 'dev': dev, 'vol_rank': vol_rank}
    ...
    if args.output:
        ...
        fname = f'bt_v4_5_volrank_{args.volume_rank}_{ts}.json'
