...
"""v4.6.2 weekly momentum scan  |  C(仙人指路)+0.02  跳过量比>1.5"""
...
# ---- v4.6.1 形态加分常量 ----
# C（仙人指路）: 低位/中段+阳线+长上影+下影短+温和量+均线多头
# B1（红三兵）: 连续3周阳线，低点抬高，量能稳定（回测显示负效果，已禁用）
C_BONUS  = 0.02   # 仙人指路加分（v2回测最优：年化+14.40%）
B1_BONUS = 0.00   # 红三兵加分已移除（回测显示有害）
...
def calc(wk):
    ...
    for i in range(MA_L, n):
        ...
        c_pattern = (
            cl[i] > wk[i]['open']           # 阳线
            and s2b > 1.0                    # 上影>实体
            and l_shadow < body * 0.5       # 下影<实体一半
            and vol_r < 1.5                  # 温和放量
            and cl[i] > ma5 > ma21          # 均线多头
            and gain20w < 0.5               # 低位/中段
        )
        b1_ok = False
        if i >= 2:
