...
def run_backtest(use_g3):
    """运行回测，use_g3=True时启用G3过滤"""
    etfs = load_pool()
    ...
    valid_weeks = [w for w in valid_weeks if w not in {'2024-W01', '2025-W01'}]
    ...
    for i in range(1, len(eq_curve)):
        ...
        returns.append(r)
    sharpe = (statistics.mean(returns) / statistics.stdev(returns)) * math.sqrt(52) if len(returns) > 1 else 0
...
print('=' * 70)
print('  对比v5.0（无G3）vs v5.0+G3')
print('=' * 70)

print('\n运行v5.0（无G3过滤）...')
result_no_g3 = run_backtest(use_g3=False)
print(f'  年化: {result_no_g3["ann_ret"]:+.1f}%')
print(f'  最大回撤: {result_no_g3["max_dd"]:.1f}%')
print(f'  夏普: {result_no_g3["sharpe"]:.2f}')

print('\n运行v5.0+G3（有G3过滤）...')
result_g3 = run_backtest(use_g3=True)
print(f'  年化: {result_g3["ann_ret"]:+.1f}%')
print(f'  最大回撤: {result_g3["max_dd"]:.1f}%')
print(f'  夏普: {result_g3["sharpe"]:.2f}')

print('\n' + '=' * 70)
print('  对比结果')
print('=' * 70)
print(f'\n{"指标":>10} {"v5.0（无G3）":>15} {"v5.0+G3":>15} {"变化":>10}')
print('-' * 60)
print(f'{"年化":>10} {result_no_g3["ann_ret"]:>+14.1f}% {result_g3["ann_ret"]:>+14.1f}% {result_g3["ann_ret"]-result_no_g3["ann_ret"]:>+9.1f}%')
print(f'{"最大回撤":>10} {result_no_g3["max_dd"]:>14.1f}% {result_g3["max_dd"]:>14.1f}% {result_g3["max_dd"]-result_no_g3["max_dd"]:>+9.1f}%')
print(f'{"夏普":>10} {result_no_g3["sharpe"]:>15.2f} {result_g3["sharpe"]:>15.2f} {result_g3["sharpe"]-result_no_g3["sharpe"]:>+14.2f}')
print(f'{"交易次数":>10} {result_no_g3["n_buys"]:>15} {result_g3["n_buys"]:>15} {result_g3["n_buys"]-result_no_g3["n_buys"]:>+14}')
print(f'{"空仓周数":>10} {result_no_g3["empty_weeks"]:>15} {result_g3["empty_weeks"]:>15} {result_g3["empty_weeks"]-result_no_g3["empty_weeks"]:>+14}')
